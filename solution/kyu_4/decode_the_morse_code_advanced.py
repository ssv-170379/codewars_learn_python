...
def decode_bits(bits):
    bits = bits.strip('0')  # clean-up leading/trailing zeroes
    time_unit = min(len(s) for s in re.findall(r'1+|0+', bits))  # find the shortest span of repeating bits, which equals time unit
    bits = bits[::time_unit]  # remove duplicates of bits when time_unit > 1
    ...
    Pause between words – is 7 time units long.
    """
    return bits.replace('111', '-').replace('1', '.').replace('0000000', '   ').replace('000', ' ').replace('0', '')  # convert bits to Morse code


def decode_morse(morse_code):
    return ' '.join(
        ''.join(MORSE_CODE[m_character] for m_character in m_word.split(' '))
        for m_word in morse_code.strip().split('   '))
